...
const map = new google.maps.Map(
    ...
    for(const i in applications){
        ...
        fetch(
            "https://maps.googleapis.com/maps/api/geocode/json?address=" + applications[i][4] + "&key=AIzaSyDovyHEqrLYWiEiqACyLfryUo7n9AmPYjg"
          )
            ...
            .then((data) => {
              if(data['results'].length < 1){
                ...
                `<br>` +
                `<br>` +
                `<p>${applications[i][5] ? ('Type: ' + applications[i][5]) : ''}</p>` +
                `<p>${applications[i][9] ? ('Status: ' + applications[i][9]) : ''}</p>` +
                `<p>${applications[i][6] ? ('Applied on ' + applications[i][6]) : ''}</p>`;
    ...
    document.getElementById('locationMap').style.height = screen.height;
  }
